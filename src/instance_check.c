    ...
    (struct launcher_data *data)
{
	...
	while(!q)
	{
		...
		if(c != ERR)
		{
			...
			case KEY_DOWN:
			{
				(data->n_instance - selected) ? ++selected : 0;
			}
			break;

			case KEY_LEFT:
			{
				selected = data->n_instance;
			}
    ...
    int selected)
{
	...
	while(!q)
	{
		...
		if( (q == 1) && selected2 != Back)
		{
			...
			case Terminate:
			{
				kill(data->instance[selected].pid, SIGTERM);
			}
			break;

			case Kill:
			{
				kill(data->instance[selected].pid, SIGKILL);
			}
			break;
